...
var numeProdus  = titluVeg.textContent;
var infoLuni = document.querySelector('.luni-sezon');
var infoLuniDiv = document.getElementsByClassName('info-luni');
...
// toggle la clasa sa apara si sa dispara cand dai click pe ea;

dropFoodDiv.addEventListener('click',function(){
    listaFood.classList.toggle('show1');
});
// event listener sa scoata clasa show cand apesi in afara divului;
window.addEventListener('click',function(e){
    if(!e.target.matches('.dropbtn1')){
    ...
    // trebuie sa afisez luna in care ne aflam si sa afisez pe pagina principala doar vegetalele ce cresc in acea luna :D;
        ...
        var data = new Date();
        var month = data.toLocaleDateString('ro-RO', {month:"long"});
        ...
        // trebuie sa afisez produsul selectat pe buton;
        var produsAf = document.querySelector('.produsAf');
        String.prototype.firstLetter = function(){
            return this.charAt(0).toUpperCase() + this.slice(1);
        }
    
        produsAf.innerHTML = numeProdus.firstLetter();

        // setez backgroundul in functie de luna;
        if(month === "ianuarie" || month ==="februarie" || month ==="decembrie"){
            imagine.style.backgroundImage = "url(img/iarna.jpg)";
        } else if(month === "martie" || month ==="aprilie" || month ==="mai" ){
            imagine.style.backgroundImage = "url(img/primavara3.jpg)";
        } else if (month === "iunie" || month ==="iulie" || month ==="august" ){
            imagine.style.backgroundImage = "url(img/vara6.jpg)";
        } else if(month === "septembrie" || month ==="octombrie" || month ==="noiembrie"){
            imagine.style.backgroundImage = "url(img/livada2.jpg)";
        }
    ...
    reqJson.open('GET','http://localhost:3000/1');
    ...
    reqJson.onreadystatechange = function(){
        ...
        date.sort(function(a,b){
            ...
            return 0;
                ...
                //  creez anchor-uri din lista de produse adica din dropdown :D
        for(let i = 0; i<date.length;i++){
            let dateSortate = date[i];
            var anchr = document.createElement('a');
            anchr.textContent = date[i].titlu;
            // calea este cea care duce la fisierul html, linkul produs duce catre retetele de gatit doar pt paginile de produse;
            anchr.setAttribute('href', date[i].cale);
            content1.appendChild(anchr);
        }
        
        //  se creeaza toate cartonasele cu datele despre produs ;
        for(let i = 0; i<date.length;i++){

            if(date[i].titlu == numeProdus){

                
                // extrag lunile pe care le are atasate produsul in JSON ;
                for(let key in date[i].luna){
                    let obj = date[i].luna[key];
                    let paraLuni = document.createElement('p');
                    paraLuni.innerText = obj.firstLetter();
                    infoLuni.appendChild(paraLuni);
                    console.log(obj);
                }
...
// trebuie sa fac lista sa se deschida la itemul selectat :D ;
